    ...
    purchase_table = $('#purchase_table').DataTable({
        ...
        "footerCallback": function ( row, data, start, end, display ) {
            ...
            for (var r in data){
                ...
                // total_quantity += $(data[r].total_quantity).data('orig-value') ? 
                // parseFloat($(data[r].total_quantity).data('orig-value')) : 0;
...
$(document).on('change', '#supplier_id', function(){
    if ($('#purchase_order_ids').length) {
     ...
     $.ajax({
            url: '/get-sells-orders/',
            dataType: 'json',
            success: function(data) {
                $('#sells_ids').select2('destroy').empty().select2({data: data});
                $('#purchase_entry_table tbody').find('tr').each( function(){
...
$("#sells_ids").on("select2:select", function (e) {
    var sells_ids = e.params.data.id;
    var row_count = $('#row_count').val();
    $.ajax({
        url: '/get-sells-order-lines/' + sells_ids + '?row_count=' + row_count,
        dataType: 'json',
        success: function(data) {
...
$("#sells_ids").on("select2:unselect", function (e) {
    var purchase_order_id = e.params.data.id;
    $('#purchase_entry_table tbody').find('tr').each( function(){
        if (typeof($(this).data('purchase_order_id')) !== 'undefined' 
