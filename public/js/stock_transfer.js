$(document).ready(function() {
    ...
    stock_transfer_table = $('#stock_transfer_table').DataTable({
        ...
        ajax: {
            "url" : '/stock-transfers',
            "data" : function ( d ) {
              if($('#tf_list_filter_date_range').val()) {
                        var start = $('#tf_list_filter_date_range').data('daterangepicker').startDate.format('YYYY-MM-DD');
                        var end = $('#tf_list_filter_date_range').data('daterangepicker').endDate.format('YYYY-MM-DD');
                        d.start_date = start;
                        d.end_date = end;
                    }
                     d.location_id_from = $('#tf_list_filter_location_id_from').val(),
                      d.location_id_to = $('#tf_list_filter_location_id_to').val(),
                      d.tf_status = $("#tf_status").val()
            }
        },
        columnDefs: [
            {
    ...
    // filter
     ...
     $('#tf_list_filter_date_range').daterangepicker(
        dateRangeSettings,
        function (start, end) {
            $('#tf_list_filter_date_range').val(start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format));
            stock_transfer_table.ajax.reload();
        }
    );
    $('#tf_list_filter_date_range').on('cancel.daterangepicker', function(ev, picker) {
        $('#tf_list_filter_date_range').val('');
        stock_transfer_table.ajax.reload();
    });
    
    $('#tf_status, #tf_list_filter_location_id_to, #tf_list_filter_location_id_from').on('change', function(){
         stock_transfer_table.ajax.reload();
    })
