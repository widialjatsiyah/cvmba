$(document).ready(function () {
    ...
    stock_report_table = $('#stock_report_table').DataTable({
        ...
        ajax: {
            ...
            data: function (d) {
                ...
                d.unit_id = $('#unit').val();
                d.product_custom_field1 = $('#konsinyasi').val();
    ...
    $(
        '#stock_report_filter_form #location_id, #stock_report_filter_form #category_id, #stock_report_filter_form #sub_category_id, #stock_report_filter_form #brand, #stock_report_filter_form #unit,#konsinyasi,#stock_report_filter_form #view_stock_filter'
    ).change(function () {
    ...
    register_report_table = $('#register_report_table').DataTable({
        ...
        columns: [
            ...
            { data: 'closing_amount', name: 'closing_amount', searchable: false },
            { data: 'selisih', name: 'selisih', searchable: false },
    ...
    $('#register_user_id, #register_status, #business_location_id').change(function () {
        updateRegisterReport();
    });
        ...
        product_sell_report = $('table#product_sell_report_table').DataTable({
            ...
            ajax: {
                ...
                data: function (d) {
                    ...
                    d.product_custom_field2 = $('select#product_custom_field2_filter').val();
                },
            },
            columns: [
                { data: 'product_name', name: 'p.name' },
    ...
    $(
        '#psr_customer_group_id, #psr_filter_category_id, #psr_filter_brand_id, #product_sell_report_form #variation_id, #product_sell_report_form #location_id, #product_sell_report_form #customer_id, #product_custom_field2_filter'
    ).change(function () {
        // alert("test");
        product_sell_report.ajax.reload();
        product_sell_grouped_report.ajax.reload();
    ...
    if ($('#ir_sale_date_filter').length == 1) {
        $('#ir_sale_date_filter').daterangepicker(dateRangeSettings, function (start, end) {
            ...
            discount_report_table.ajax.reload();
        });
        $('#ir_sale_date_filter').on('cancel.daterangepicker', function (ev, picker) {
            $('#ir_sale_date_filter').val('');
            discount_report_table.ajax.reload();
    ...
    discount_report_table = $('#discount_report_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: '/reports/items-claim-report', // sesuaikan dengan route Anda
            data: function (d) {
                var sale_start = '';
                var sale_end = '';
                if ($('#ir_sale_date_filter').val()) {
        ...
        columns: [
            ...
            { data: 'discount_claim', name: 'discount_claim' },
        ...
        fnDrawCallback: function (oSettings) {
            ...
            $('#footer_total_discount').html(
                sum_table_col($('#discount_report_table'), 'discount_claim')
            );
            // Total kuantitas
            $('#footer_total_qty').html(sum_table_col($('#discount_report_table'), 'quantity'));
            // Jika ada format mata uang, jalankan converter
            __currency_convert_recursively($('#discount_report_table'));
        },
    });

    // Event reload saat filter berubah
    $(document).on('change', '#ir_supplier_id, #ir_customer_id, #ir_location_id', function () {
        discount_report_table.ajax.reload();
    });
    // End claim report
...
function updateRegisterReport() {
    ...
    var data = {
        ...
        start_date: start,
        end_date: end,
        business_location_id: $('#business_location_id').val(),
    };
    var out = [];
...
function get_stock_value() {
    ...
    var data = {
        ...
        brand_id: $('#brand').val(),
        unit_id: $('#unit').val(),
        product_custom_field1: $('#konsinyasi').val(),
    };
    $.ajax({
        url: '/reports/get-stock-value',
        data: data,
        beforeSend: function () {
            $('.location_id_filter').prop('disabled', true);
        },
        success: function (data) {
            ...
            $('.location_id_filter').prop('disabled', false);
        },
    });
}
