$(document).ready(function() {
    ...
    $(document).on('keyup keypress', function(e) {
        ...
        if (keyCode==43 || keyCode==40 ) { // 40 = Arrow Down
            // console.log("Ctrl Down ditekan");
    ...
    if ($('#search_product').length) {
        ...
        $('#search_product')
            .autocomplete({
                ...
                select: function(event, ui) {
                    ...
                    if (ui.item.enable_stock != 1 || ui.item.qty_available > 0 || is_overselling_allowed || for_so || is_draft ) {
                        ...
                        if(ui.item.variation_group_price){
                             pos_product_row(ui.item.variation_id, purchase_line_id);
                            
                        }else if(typeof ui.item.variation_group_price  === "undefined"){
                             pos_product_row(ui.item.variation_id, purchase_line_id);
                        }else{
                             toastr.error("Harga Group Belum Diatur");
                        }
                       
                    } else {
                        alert(LANG.out_of_stock+' Atau Harga Group Belum diatur');
                    }
                },
            })
            ...
            if (item.enable_stock == 1 && item.qty_available <= 0 && !is_overselling_allowed && !for_so && !is_draft && item.variation_group_price) {
                ...
                if (item.variation_group_price) {
                    ...
                    string += ' <br><b>Harga Group Belum di Atur</b>';
                }
                string +=
                    ' (' +
                    item.sub_sku +
                ...
                if (item.variation_group_price) {
                    selling_price = item.variation_group_price;
                }else if(typeof item.variation_group_price === "undefined"){
                    string += '';
                }else{
                    string += ' <b>Harga Group Belum di Atur</b>';
                }
    ...
    $('table#pos_table').on('change', 'select.sub_unit', function() {
        ...
        var base_unit_selling_price = parseFloat(selected_option.data('unit_harga'));
        ...
        var unit_sp = base_unit_selling_price;
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.success) {
                    ...
                    toastr.error(result.msg+' Atau Harga Group Belum Diatur');
                    $('input#search_product')
                        .focus()
                        .select();
                }
...
function calculate_discounted_unit_price_old(row) {
    ...
    if (discount_input) {
        if (row_discount_type == 'fixed') {
            ...
            row_discounted_unit_price =
                this_unit_price - total_discount;

        } else {

            row_discounted_unit_price =
                applyTieredDiscount(this_unit_price, discount_input);
...
function get_unit_price_from_discounted_unit_price(
    ...
    if (discount_input) {
        ...
        if (row_discount_type == 'fixed') {
            ...
            this_unit_price =
                discounted_unit_price + total_discount;

        } else {

            this_unit_price =
                reverseTieredDiscount(
                    discounted_unit_price,
                    discount_input
                );
...
function get_unit_price_from_discounted_unit_price_old(row, discounted_unit_price) {
    var this_unit_price = discounted_unit_price;
    var row_discount_type = row.find('select.row_discount_type').val();
    var row_discount_amount = __read_number(row.find('input.row_discount_amount'));
    if (row_discount_amount) {
...
$(document).on('change', '.payment_types_dropdown', function(e) {
    ...
    if (payment_type && payment_type != 'advance') {
        ...
        var account_return = payment_row.find('select#change_return_account');
        account_return.val(default_account);
        account_return.change();
...
$(document).on('click', '#send_pin_staff_verify', function(e) {
    var pin_staff = $('#pin_staff_verify').val();

    if (pin_staff) {
        $.ajax({
            method: 'get',
            url: /validate-staff-pin/ + encodeURI(pin_staff),
            dataType: 'json',
            success: function(result) {
                // console.log(result);
                if (result == true) {
                     $('#return_sale').popover('hide');
                       swal({
                        text: 'Scan / Masukan No Struk',
                        buttons: true,
                        dangerMode: true,
                        content: {
                            element: "input",
                    ...
                    .then((inputValue) => {
                        if (inputValue !== null) {
                                 $.ajax({
                                    method: 'get',
                                    url: '/validate-invoice-to-return/' + encodeURI(inputValue),
                                    dataType: 'json',
                                    success: function(result) {
                                        if (result.success == true) {
                                            // window.location = result.redirect_url ;
                                             window.open(result.redirect_url,'NewWin',
            'toolbar=no,status=no,width=850,height=450,resizable=yes')
                                        } else {
                                            toastr.error(result.msg);
                                        }
                                    },
