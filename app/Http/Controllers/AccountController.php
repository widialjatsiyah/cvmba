        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($accounts)
                            ->addColumn(
                                ...
                                <button data-href="{{action(\'App\Http\Controllers\AccountController@getFundTransfer\',[$id])}}" class="btn btn-xs btn-info btn-modal" data-container=".view_modal"><i class="fas fa-exchange-alt"></i> @lang("account.fund_transfer")</button>

                                <button data-href="{{action(\'App\Http\Controllers\AccountController@getDeposit\',[$id])}}" class="btn btn-xs btn-success btn-modal" data-container=".view_modal"><i class="fas fa-plus"></i> Penyesuaian</button>
                                ...
                                <a href="{{action(\'App\Http\Controllers\AccountController@show\',[$id])}}" class="btn btn-warning btn-xs"><i class="fa fa-book"></i> Buku Kas</a>&nbsp;
                            ...
                            ->editColumn('parent_account_type_name', function ($row) {
                                $parent_account_type_name = empty($row->parent_account_type_name) 
                                    ? $row->account_type_name 
                                    : '<b>'.$row->parent_account_type_name . '</b><br>' . $row->account_type_name;
                            ...
                            ->removeColumn('account_type_name')
                            ->removeColumn('id')
                            ->removeColumn('is_closed')
                            ->rawColumns(['action', 'balance', 'name', 'account_details','parent_account_type_name'])
                            ->make(true);
        }
    ...
    public function show($id)
{
    ...
    if (request()->ajax()) {
        ...
        if (!empty($type)) {
            $before_bal_query->where('account_transactions.type', $type);
        }

        $bal_before_start_date = optional(
            $before_bal_query->first()
        )->prev_bal ?? 0;
        ...
        $accounts = AccountTransaction::query()
            ...
            ->select([
                ...
                DB::raw('COALESCE(SUM(child_payments.amount),0) as total_recovered'),
                DB::raw('GROUP_CONCAT(child_sells.invoice_no) as child_sells'),

                /**
                 * Running balance (PENGGANTI N+1 QUERY)
                 */
                DB::raw("
                    (
                        SELECT COALESCE(
                        ...
                        WHERE at2.account_id = account_transactions.account_id
                        AND at2.deleted_at IS NULL
                        AND at2.operation_date >= '{$start_date}'
                        AND at2.operation_date <= account_transactions.operation_date
                    ) as running_balance
                ")
            ])
        ...
        if (!empty($type)) {
            $accounts->where('account_transactions.type', $type);
        }
        ...
        return DataTables::of($accounts)
            ...
            ->editColumn('method', function ($row) use ($payment_types) {
                return !empty($row->method) && isset($payment_types[$row->method])
                    ? $payment_types[$row->method]
                    : '';
            })
            ...
            ->addColumn('payment_details', function ($row) {
                ...
                if ($row->method == 'card') {
                    ...
                    if (!empty($row->card_holder_name)) {
                        $arr[] = '<b>'.__('lang_v1.card_holder_name').'</b>: '.$row->card_holder_name;
                    }

                    if (!empty($row->card_month)) {
                        $arr[] = '<b>'.__('lang_v1.month').'</b>: '.$row->card_month;
                    }

                    if (!empty($row->card_year)) {
                        $arr[] = '<b>'.__('lang_v1.year').'</b>: '.$row->card_year;
                    }

                    if (!empty($row->card_security)) {
                        $arr[] = '<b>'.__('lang_v1.security_code').'</b>: '.$row->card_security;
                    }
                }
            ...
            ->addColumn('debit', function ($row) {
                return $row->type == 'debit'
                    ? '<span class="debit" data-orig-value="'.$row->amount.'">'.$this->commonUtil->num_f($row->amount, true).'</span>'
                    : '';
            })

            ->addColumn('credit', function ($row) {
                return $row->type == 'credit'
                    ? '<span class="credit" data-orig-value="'.$row->amount.'">'.$this->commonUtil->num_f($row->amount, true).'</span>'
                    : '';
            })

            ->addColumn('balance', function ($row) use ($bal_before_start_date) {

                $bal = $bal_before_start_date + $row->running_balance;

                return '<span class="balance" data-orig-value="'.$bal.'">'.
                    $this->commonUtil->num_f($bal, true).
                    '</span>';
            })
            ...
            ->editColumn('action', function ($row) {
                ...
                if (auth()->user()->can('delete_account_transaction')) {
                    if (in_array($row->sub_type, ['fund_transfer', 'deposit'])) {
                        $action .= '<button type="button"
                            class="btn btn-danger btn-xs delete_account_transaction"
                            data-href="'.
                            action([\App\Http\Controllers\AccountController::class,
                            'destroyAccountTransaction'], [$row->id])
                            .'">
                ...
                if (auth()->user()->can('edit_account_transaction')) {
                    if (in_array($row->sub_type, ['fund_transfer', 'deposit', 'opening_balance'])) {
                        $action .= ' <button type="button"
                            class="btn btn-primary btn-xs btn-modal"
                            data-container="#edit_account_transaction"
                            data-href="'.
                            action([\App\Http\Controllers\AccountController::class,
                            'editAccountTransaction'], [$row->id])
                            .'">
                ...
                $media = $row->media->first()
                    ?? optional($row->transfer_transaction)->media->first();
    ...
    public function __getPaymentDetails($row)
    {
        ...
        if (! empty($row->sub_type)) {
            if($row->sub_type=="deposit"){
                $namaDana = "Penyesuaian";
            }else{
                $namaDana = __('account.'.$row->sub_type);
            }
            $details = $namaDana;
