...
class LabelsController extends Controller
{
    ...
    public function show(Request $request)
    {
          ...
          $allowPrice = [];
            foreach ($price_groups as $key => $value) {
                if (auth()->user()->can('selling_price_group.' . $key)) {
                    $allowPrice[$key] = $value;
                }
            }
            $key = array_key_first($allowPrice);
        ...
        return view('labels.show')
            ->with(compact('products', 'barcode_settings', 'default', 'price_groups','key'));
    }
          ...
          $allowPrice = [];
            ...
            $key = array_key_first($allowPrice);
                ...
                return view('labels.partials.show_table_rows')
                        ->with(compact('products', 'index', 'price_groups','key'));
            }
        }
    }
        ...
        try {
            ...
            foreach ($products as $value) {
                ...
                if (! empty($value['price_group_id'])) {
                    ...
                    $group_prices = $this->productUtil->getVariationGroupPriceBaru($value['variation_id'], $value['price_group_id'], $tax_id);
                    $details->group_prices =  $group_prices;
