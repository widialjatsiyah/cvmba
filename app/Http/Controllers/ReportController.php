...
class ReportController extends Controller
{
    ...
    public function getRegisterReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $registers = $this->transactionUtil->registerReport($business_id, $permitted_locations, $start_date, $end_date, $business_location_id);
            ...
            return Datatables::of($registers)
                ...
                //     return '<span data-orig-value="'.$row->total_custom_pay_7.'" >'.$this->transactionUtil->num_f$row->total_custom_pay_7, true, true).'</span>';
                // })
                ...
                ->editColumn('closing_amount', function ($row) {
                    return '<span data-orig-value="' . $row->closing_amount . '" >' . $this->transactionUtil->num_f($row->closing_amount, true) . '</span>';
                })
                ->addColumn('selisih', function ($row) {
                    $total = $row->total_cash_payment;
                    $closing_am = $row->closing_amount;
                    $selisih =  $closing_am -  $total;
                    return '<span data-orig-value="' . $selisih . '" >' . $this->transactionUtil->num_f($selisih, true) . '</span>';
                })
                ...
                ->rawColumns(['action', 'user_name',  'total_cash_payment', 'total_bank_transfer_payment', 'total_other_payment', 'total_advance_payment', 'total_custom_pay_1', 'total_custom_pay_2', 'total', 'closing_amount', 'selisih'])
                ->make(true);
        }
    ...
    public function getproductSellReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $product_custom_field2_filter = $request->get('product_custom_field2', null);
            if (! empty($product_custom_field2_filter)) {
                $query->where('p.product_custom_field2', $product_custom_field2_filter);
            }
    ...
    public function productSellReportBy(Request $request)
    {
        ...
        $product_custom_field2 = $request->get('product_custom_field2', null);
        ...
        if ($request->ajax()) {
            ...
            if (! empty($product_custom_field2)) {
                $query->where('p.product_custom_field2', $product_custom_field2);
            }
    ...
    public function itemsClaimReport()
    {
        $business_id = request()->session()->get('user.business_id');

        if (request()->ajax()) {
            $query = TransactionSellLine::leftJoin('transactions as sale', 'transaction_sell_lines.transaction_id', '=', 'sale.id')
                ->join('variations as v', 'transaction_sell_lines.variation_id', '=', 'v.id')
                ->join('product_variations as pv', 'v.product_variation_id', '=', 'pv.id')
                ->join('products as p', 'v.product_id', '=', 'p.id')
                ->join('units as u', 'p.unit_id', '=', 'u.id')
                ->leftJoin('contacts as suppliers', 'p.product_custom_field2', '=', 'suppliers.id')
                ->where('sale.business_id', $business_id)
                ->where('sale.type', 'sell') // hanya transaksi penjualan
                ->select(
                    'v.sub_sku as sku',
                    'p.name as product_name',
                    'p.type as product_type',
            ...
            $supplier_id = request()->get('supplier_id', null);
            if (! empty($supplier_id)) {
                $query->where('p.product_custom_field2', $supplier_id);
            }

            // Filter pelanggan
            if (!empty(request()->customer_id)) {
                $query->where('sale.contact_id', request()->customer_id);
            }
            ...
            return Datatables::of($query)
                ...
                ->editColumn('invoice_no', function ($row) {
                    // Link ke detail penjualan jika perlu
                ...
                ->addColumn('discount_claim', function ($row) {
                    ...
                    $claim_discount = 0;
                    if (!empty($row->line_discount_type_claim) && $row->line_discount_amount_claim > 0) {
                        if ($row->line_discount_type_claim == 'percentage') {
                            $claim_discount = ($row->line_discount_amount_claim / 100) * $price_after_regular_discount;
                        } else { // fixed
                            $claim_discount = (float) $row->line_discount_amount_claim;
                        }
                    }

                    return '<span class="display_currency discount_claim" data-currency_symbol="true" data-orig-value="' . $claim_discount . '">' . $this->transactionUtil->num_f($claim_discount, true) . '</span>';
                })

                ->editColumn('quantity', function ($row) {
                    return '<span class="display_currency quantity" data-currency_symbol="false" data-orig-value="' . (float) $row->quantity . '">' . (float) $row->quantity . '</span> ';
                })
                ->editColumn('sell_date', '{{@format_datetime($sell_date)}}')
                ->editColumn('supplier_name', function ($row) {
                    return $row->supplier_business_name ? $row->supplier_business_name : $row->supplier_name;
                })

                ->rawColumns(['invoice_no', 'discount_claim', 'quantity', 'supplier_name'])
                ->make(true);
        }

        // Untuk tampilan (view) ambil data dropdown jika diperlukan

        $suppliers = Contact::suppliersDropdown($business_id, false);
        $customers = Contact::customersDropdown($business_id, false);
        $business_locations = BusinessLocation::forDropdown($business_id);

        return view('report.items_claim_report')->with(compact('customers', 'business_locations', 'suppliers'));
    }
    /**
    ...
    public function getStockValue()
    {
        ...
        $location_id = request()->input('location_id');
        $filters = request()->only(['category_id', 'sub_category_id', 'brand_id', 'unit_id', 'product_custom_field1']);
