...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            //Home
            $menu->url(action([\App\Http\Controllers\HomeController::class, 'index']), __('home.home'), ['icon' => 'ion ion-ios-speedometer-outline', 'active' => request()->segment(1) == 'home'])->order(5);
            // $menu->url(action([\App\Http\Controllers\ReportdayController::class, 'index']), __('Report Day'), ['icon' => 'fa fas fa-clipboard', 'active' => request()->segment(1) == 'report_day'])->order(5);
                    ...
                    function ($sub) {
                        if (auth()->user()->can('user.view')) {
                            $sub->url(
                                ...
                                __('user.users'),
                                ['icon' => 'ion ion-ios-people-outline', 'active' => request()->segment(1) == 'users']
                            );
                        }
            ...
            if (auth()->user()->can('supplier.view') || auth()->user()->can('customer.view') || auth()->user()->can('supplier.view_own') || auth()->user()->can('customer.view_own')) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('customer.view') || auth()->user()->can('customer.view_own')) {
                            $sub->url(
                                ...
                                __('report.customer'),
                                ['icon' => 'fa fas ', 'active' => request()->input('type') == 'customer']
                            );
                            $sub->url(
                                action([\App\Http\Controllers\CustomerGroupController::class, 'index']),
                                __('lang_v1.customer_groups'),
                                ['icon' => 'fa fas ', 'active' => request()->segment(1) == 'customer-group']
                            );
                        }
                    ...
                    ['icon' => 'ion ion-ios-list-outline', 'id' => 'tour_step4']
                )->order(15);
            }
            ...
            if (auth()->user()->can('product.view') || auth()->user()->can('product.create') ||
                ...
                $menu->dropdown(
                    ...
                    ['icon' => 'ion ion-ios-pricetags-outline', 'id' => 'tour_step5']
                )->order(20);
            }
            ...
            if (in_array('purchases', $enabled_modules) && (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create') || auth()->user()->can('purchase.update'))) {
                $menu->dropdown(
                    ...
                    ['icon' => 'ion ion-ios-undo-outline', 'id' => 'tour_step6']
                )->order(25);
            }
            //Sell dropdown
            if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping', 'access_sell_return', 'direct_sell.view', 'direct_sell.update', 'access_own_sell_return'])) {
                $menu->dropdown(
                    ...
                    ['icon' => 'ion ion-ios-redo-outline', 'id' => 'tour_step7']
                )->order(30);
            }

            //Stock transfer dropdown
            if (in_array('stock_transfers', $enabled_modules) && (auth()->user()->can('transfer_stock.view') || auth()->user()->can('transfer_stock.create'))) {
                $menu->dropdown(
                    __('lang_v1.stock_transfers'),
                    function ($sub) {
                        if (auth()->user()->can('transfer_stock.view')) {
                            $sub->url(
                        ...
                        if (auth()->user()->can('transfer_stock.create')) {
                            $sub->url(
            ...
            if (in_array('stock_adjustment', $enabled_modules) && (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create'))) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        if (auth()->user()->can('purchase.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\StockOpnameController::class, 'index']),
                                __('List Stock Opname'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'stock-opname' && request()->segment(2) == null]
                            );
                        }
                        if (auth()->user()->can('purchase.create')) {
                            $sub->url(
                                action([\App\Http\Controllers\StockOpnameController::class, 'create']),
                                __('Tambah Stock Opname'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'stock-opname' && request()->segment(2) == 'create']
                            );
                        }
                    },
                    ['icon' => 'ion ion-ios-paper-outline']
            ...
            if (auth()->user()->can('purchase_n_sell_report.view') || auth()->user()->can('contacts_report.view')
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        ...
                        if (auth()->user()->can('contacts_report.view')) {
                            $sub->url(
                                ...
                                __('report.contacts'),
                                ['icon' => 'ion ion-ios-list-outline', 'active' => request()->segment(2) == 'customer-supplier']
                            );
                            $sub->url(
                    ...
                    ['icon' => 'ion ion-ios-pie-outline', 'id' => 'tour_step8']
                )->order(55);
            }
            ...
            if (auth()->user()->can('send_notifications')) {
                $menu->url(action([\App\Http\Controllers\NotificationTemplateController::class, 'index']), __('lang_v1.notification_templates'), ['icon' => 'ion ion-android-mail', 'active' => request()->segment(1) == 'notification-templates'])->order(80);
            }
            ...
            if (auth()->user()->can('business_settings.access') ||
                ...
                $menu->dropdown(
                    ...
                    ['icon' => 'ion ion-ios-gear-outline', 'id' => 'tour_step3']
                )->order(85);
            }
        });
