...
class TransactionUtil extends Util
{
                ...
                if (! empty($payments)) {
                    foreach ($payments as $value) {
                        ...
                        if ($value['method'] == 'cash') {
                            $output['payments'][] =
                                [
                                    'method' => ($value['is_return'] == 1 ? ' (' . $il->change_return_label . ')' : $method),
    ...
    public function getSellTotalsMobile($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null, $permitted_locations = null)
    {
        ...
        // Filter untuk lokasi yang diizinkan
        // if (!empty($permitted_locations)) {
        //     if ($permitted_locations != 'all') {
        //         $query->whereIn('transactions.location_id', $permitted_locations);
        //     }
        ...
        // Ambil total berdasarkan lokasi
        $sell_dc = (clone $query)->where('transactions.location_id', 1)->first();
        $sell_mart = (clone $query)->where('transactions.location_id', 2)->first();
        $sell_grosir = (clone $query)->where('transactions.location_id', 3)->first();
        $sell_lepari = (clone $query)->where('transactions.location_id', 4)->first();

        // Output data
        $output['total_sell_inc_tax'] = $sell_details->total_sell ?? 0;
        $output['total_sell_inc_tax_dc'] = $sell_dc->total_sell ?? 0;
        $output['total_sell_inc_tax_mart'] = $sell_mart->total_sell ?? 0;
        $output['total_sell_inc_tax_grosir'] = $sell_grosir->total_sell ?? 0;
        $output['total_sell_inc_tax_lepari'] = $sell_lepari->total_sell ?? 0;
    ...
    $purchase_line_id = null
        ...
        if ($check_expiry) {
            if (
                session()->has('business') &&
                request()->session()->get('business')['enable_product_expiry'] == 1 &&
                request()->session()->get('business')['on_product_expiry'] == 'stop_selling'
            ) {
    ...
    public function mapPurchaseSell_old($business, $transaction_lines, $mapping_type = 'purchase', $check_expiry = true, $purchase_line_id = null)
    {
        ...
        foreach ($transaction_lines as $line) {
            ...
            foreach ($rows as $k => $row) {
                $qty_allocated = 0;
                $qty_ada = $row->quantity_available;
            ...
            if (! ($qty_selling == 0 || is_null($qty_selling))) {
                ...
                if (! $allow_overselling) {
                    ...
                    $mismatch_name = $product->name . '[' . $product->id . ']';
                    if (! empty($variation->sub_sku)) {
                        $mismatch_name .= ' ' . 'SKU: ' . $variation->sub_sku;
                    }
                    if (! empty($qty_selling)) {
                        $mismatch_name .= ' ' . 'Quantity: ' . abs($qty_selling) . '/ ' . $line->quantity . '/ ' . $rows;
                    }
    ...
    public function getOpeningClosingStock($business_id, $date, $location_id, $is_opening = false, $by_sale_price = false, $filters = [], $permitted_locations = null)
    {
        ...
        if (isset($filters['product_custom_field1'])) {
            if ($filters['product_custom_field1'] == 1) {
                $query->where(function ($q) {
                    $q->where('p.product_custom_field1', '!=', 0)
                        ->orWhereNull('p.product_custom_field1') // Memasukkan NULL
                        ->orWhere('p.product_custom_field1', ''); // Memasukkan string kosong ''
                });
            } else {
                $query->where('p.product_custom_field1', 0);
            }
        }
    ...
    public function recalculateSellLineTotals($business_id, $sell_line)
    {
        ...
        if (! empty($sub_unit_id)) {
            $multiplier = ($sub_unit_id != '') ? $sell_line->konversi : 1;
    ...
    public function recalculateSellLineTotalsOld($business_id, $sell_line)
    {
        $unit_details = $this->getSubUnits($business_id, $sell_line->product->unit->id);
    ...
    public function getTransactionTotals(
        ...
        if (in_array('expense', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(IF(transactions.type='expense' AND expense_category_id != 16, final_total, 0)) as total_expense")
            );
    ...
    public function getListPurchases($business_id)
    {
        $purchases = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ...
            ->select(
                ...
                DB::raw('(SELECT SUM(tp2.amount) 
                      FROM transaction_payments AS tp2 
                      WHERE tp2.transaction_id = transactions.id) as amount_paid'),
    ...
    public function getListSells($business_id, $sale_type = 'sell')
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ...
            ->select(
                ...
                DB::raw('(SELECT GROUP_CONCAT(COALESCE(TP3.method, "") SEPARATOR ", ") 
                              FROM transaction_payments AS TP3 
                              WHERE TP3.transaction_id = transactions.id) AS method')
            );
    ...
    public function createExpense_old($request, $business_id, $user_id, $format_data = true)
    {
    ...
    {
        // Ambil data expense lines dari request
        $expense_lines = $request->input('expense_lines', []);

        // Hitung total dari semua line
        $total_amount = 0;
        foreach ($expense_lines as $line) {
            $amount = $format_data ? $this->num_uf($line['amount']) : $line['amount'];
            $total_amount += $amount;
        }
        ...
        $transaction_data['created_by'] = $user_id;
        $transaction_data['type'] = !empty($request->input('is_refund')) && $request->input('is_refund') == 1 ? 'expense_refund' : 'expense';
        $transaction_data['expense_category_id'] = !(empty($request->input('expense_category_id'))) ? $request->input('expense_category_id') : null;
        $transaction_data['status'] = 'final';
        $transaction_data['payment_status'] = 'due';
        $transaction_data['final_total'] = $total_amount;

        // Tax global (jika ada)
        if (!empty($transaction_data['tax_id'])) {
            $tax_details = TaxRate::find($transaction_data['tax_id']);
            $transaction_data['total_before_tax'] = $this->calc_percentage_base($total_amount, $tax_details->amount);
            $transaction_data['tax_amount'] = $total_amount - $transaction_data['total_before_tax'];
        } else {
            $transaction_data['total_before_tax'] = $total_amount;
        ...
        if ($request->has('is_recurring')) {
            $transaction_data['is_recurring'] = 1;
            $transaction_data['recur_interval'] = $request->input('recur_interval', 1);
            $transaction_data['recur_interval_type'] = $request->input('recur_interval_type');
            $transaction_data['recur_repetitions'] = $request->input('recur_repetitions');
            $transaction_data['subscription_repeat_on'] = $request->input('subscription_repeat_on');
        }
        ...
        // Simpan expense lines
        foreach ($expense_lines as $line) {
            $expense_line = new \App\TransactionExpense(); // Sesuaikan namespace model Anda
            $expense_line->transaction_id = $transaction->id;
            $expense_line->expense_category_id = !empty($line['category_id']) ? $line['category_id'] : null;
            $expense_line->expense_sub_category_id = !empty($line['sub_category_id']) ? $line['sub_category_id'] : null;
            $expense_line->expense_for = !empty($line['expense_for']) ? $line['expense_for'] : null;
            $expense_line->source_ref_no = !empty($line['source_ref_no']) ? $line['source_ref_no'] : null;
            $expense_line->line_description = $line['line_description'] ?? null;
            $expense_line->amount = $format_data ? $this->num_uf($line['amount']) : $line['amount'];
            $expense_line->save();
        }
    ...
    public function updateExpense($request, $id, $business_id, $format_data = true)
    {
        ...
        // Ambil expense lines dari request
        $expense_lines = $request->input('expense_lines', []);
        ...
        // Tax
        $tax_id = $request->has('tax_id') ? $request->input('tax_id') : $transaction->tax_id;
        if (!empty($tax_id)) {
        ...
        $transaction_data['recur_interval'] = $request->input('recur_interval', 0);
        $transaction_data['recur_interval_type'] = $request->input('recur_interval_type');
        $transaction_data['recur_repetitions'] = $request->input('recur_repetitions');
        $transaction_data['subscription_repeat_on'] = $request->input('subscription_repeat_on');
        ...
        // ========== PROSES EXPENSE LINES ==========
        // Hapus semua lines yang lama
        $transaction->expenseLines()->delete();

        // Simpan lines yang baru
        foreach ($expense_lines as $line) {
            $amount = $format_data ? $this->num_uf($line['amount']) : $line['amount'];
            if (empty($amount)) continue; // skip baris dengan amount 0 atau kosong

            $expense_line = new \App\TransactionExpense();
            $expense_line->transaction_id = $transaction->id;
            $expense_line->expense_category_id = !empty($line['category_id']) ? $line['category_id'] : null;
            $expense_line->expense_sub_category_id = !empty($line['sub_category_id']) ? $line['sub_category_id'] : null;
            $expense_line->amount = $amount;
            $expense_line->line_description = $line['line_description'] ?? null;
            $expense_line->save();
        }
    ...
    public function updateExpense_old($request, $id, $business_id, $format_data = true)
    {
        $transaction_data = [];
     ...
     * @return array
     */
    public function registerReport($business_id, $permitted_locations, $start_date = null, $end_date = null, $business_location_id = null)
    {
        ...
        if (!empty($permitted_locations)) {
            if ($permitted_locations != 'all' && is_null($business_location_id)) {
                $registers->whereIn('cash_registers.location_id', $permitted_locations);
            } else if (!is_null($business_location_id)) {
                $registers->where('cash_registers.location_id', $business_location_id);
            }
        }
