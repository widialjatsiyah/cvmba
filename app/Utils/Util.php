...
class Util
{
    ...
    public function payment_types_filter($location = null, $show_advance = false, $business_id = null)
    {
        if (! empty($location)) {
            $location = is_object($location) ? $location : BusinessLocation::find($location);
    ...
    //  MASTER SATUAN
     public function getSatuanProduk($business_id, $price_group, $product_id = null)
    {
        
        $results = DB::table('variation_group_prices as vgp')
                ->leftJoin('variations as v', 'vgp.variation_id', '=', 'v.id')
                ->leftJoin('products as p', 'p.id', '=', 'v.product_id')
                ->leftJoin('units as u', 'vgp.unit_id', '=', 'u.id')
                ->select(
                    'u.actual_name',
                    'u.short_name',
                    'vgp.*',
                    ...
                    'p.name',
                    'p.unit_id as unit_id_def',
                    'u.allow_decimal'
                )
                ->where('vgp.price_group_id', $price_group)
                ->where('v.product_id', $product_id)  // Adding the condition here
                ->get();
        ...
        if (count($results) > 0) {
            foreach ($results as $unit) {
                $is_main = ($unit->unit_id==$unit->unit_id_def)?'1':'0';
                //Check if subunit is related to the product or not.
                    $sub_units[$unit->unit_id] = [
                        'name' => $unit->actual_name,
                        'multiplier' => $unit->konversi,
                        'allow_decimal' => $unit->allow_decimal,
                        'harga' => $unit->price_inc_tax,
                        'is_main' =>  ($unit->is_main==1)?$unit->is_main:$is_main,
                    ];
            }
        }
