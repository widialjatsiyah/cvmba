...
class ProductUtil extends Util
{
     ...
     * @return decimal
     */
    public function getVariationGroupPriceBaru($variation_id, $price_group_id, $tax_id)
    {
        $price_group = VariationGroupPrice::where('variation_id', $variation_id)
                        ->where('price_group_id', $price_group_id)
                        ->join('units', 'variation_group_prices.unit_id', '=', 'units.id')
                        ->select(['price_inc_tax', 'price_type','actual_name'])
                        ->get();
     ...
     */
     ...
     public function createOrUpdatePurchaseLinesNew($transaction, $input_data, $currency_details, $enable_product_editing,$c_supplier, $before_status = null)
     {
         ...
         foreach ($input_data as $data) {
             ...
             if ($enable_product_editing == 1 && $transaction->type == 'purchase' ) {
                 if (!in_array($c_supplier, ["4", "5", "27", "28", "50"])) {
                     ...
                     $this->updateSupplierProduct($c_supplier,$data['product_id']);
                    }
             }
    ...
    public function updateSupplierProduct($c_supplier,$product_id){
        $product = Product::find($product_id);
        $product->product_custom_field2 = $c_supplier;
        $product->save();
    }
   ...
   public function updatePurchaseOrderLine($purchase_order_line_id, $new_qty, $old_qty = 0)
{
    ...
    public function getVariationStockDetails($business_id, $variation_id, $location_id)
    {
        $purchase_details = Variation::join('products as p', 'p.id', '=', 'variations.product_id')
                    ...
                    // ->leftjoin('stock_opname_lines as sol', 'sol.variation_id', '=', 'variations.id')
                    ...
                    ->select(
                        ...
                        // DB::raw("SUM(IF(t.type='stock_opname', sol.difference, 0)) as total_diff"),
        ...
        $so_details = Variation::join('products as p', 'p.id', '=', 'variations.product_id')
    ->join('units', 'p.unit_id', '=', 'units.id')
    ->leftJoin('units as u', 'p.secondary_unit_id', '=', 'u.id')
    ->leftJoin('product_variations as pv', 'variations.product_variation_id', '=', 'pv.id')
    ...
    // STOCK OPNAME
    ->leftJoin('stock_opname_lines as sol', 'sol.variation_id', '=', 'variations.id')
    ->leftJoin('transactions as t_sol', 'sol.transaction_id', '=', 't_sol.id')

    ->where('p.business_id', $business_id)
    ->where('variations.id', $variation_id)

    ->where(function ($q) use ($location_id) {
        $q->where('t_sol.location_id', $location_id);
    })

    ->select(

        DB::raw("
            SUM(IF(t_sol.type='stock_opname', 
                sol.difference, 0)) 
            as total_diff
        "),
    ...
    ->first();
        ...
        $output = [
            ...
            'total_diff' => $so_details->total_diff,
    ...
    public function getVariationStockHistory($business_id, $variation_id, $location_id)
    {
        $stock_history = Transaction::leftjoin('transaction_sell_lines as sl',
            'sl.transaction_id', '=', 'transactions.id')
                                ...
                                ->leftjoin('stock_opname_lines as sol',
                                    'sol.transaction_id', '=', 'transactions.id')
                                ...
                                ->where(function ($q) use ($variation_id) {
                                    $q->where('sl.variation_id', $variation_id)
                                        ...
                                        ->orWhere('sol.variation_id', $variation_id)
                                        ->orWhere('rpl.variation_id', $variation_id)
                                        ->orWhere('rsl.variation_id', $variation_id);
                                })
                                ->whereIn('transactions.type', ['sell', 'purchase', 'stock_adjustment', 'opening_stock', 'sell_transfer', 'purchase_transfer', 'production_purchase', 'purchase_return', 'sell_return', 'production_sell','stock_opname'])
                                ->select(
        ...
        foreach ($stock_history as $stock_line) {
            ...
            } elseif ($stock_line->transaction_type == 'stock_opname') {
                $quantity_change = $stock_line->stock_diff;
                $stock += $quantity_change;
                $stock_history_array[] = array_merge($temp_array, [
                    'quantity_change' => $quantity_change,
                    'stock' => $this->roundQuantity($stock),
                    'type' => 'stock_opname',
                    'type_label' => 'Penyesuaian Stock Opname',
                    'ref_no' => $stock_line->ref_no,
                    'stock_in_second_unit' => $this->roundQuantity($stock_in_second_unit),
                ]);
    ...
    public function getVariationStockMisMatch($business_id, $variation_id, $location_id)
    {
        ...
        $stock_details = $query->select(
            ...
            DB::raw("(SELECT SUM(COALESCE(SAL.quantity, 0)) FROM transactions 
                    ...
                    AND SAL.variation_id=variations.id) as total_adjusted"),
            DB::raw("(SELECT SUM(COALESCE(SOL.difference, 0)) FROM transactions 
                    LEFT JOIN stock_opname_lines AS SOL ON transactions.id=SOL.transaction_id
                    WHERE transactions.type='stock_opname' AND transactions.location_id=$location_id 
                    AND SOL.variation_id=variations.id) as total_diff"),
            DB::raw("(SELECT SUM(COALESCE(PL.quantity, 0)) FROM transactions 
                    LEFT JOIN purchase_lines AS PL ON transactions.id=PL.transaction_id
        ...
        foreach ($stock_details as $index => $row) {
            ...
            $total_diff_so = $row->total_diff ?: 0;
            ...
            $total_stock_calculated = $total_opening_stock + $total_purchased + $total_purchase_transfered + $total_sell_return + $total_manufactured + ($total_diff)
