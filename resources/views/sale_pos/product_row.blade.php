...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	<td>
		...
		<span class="text-link text-dark cursor-pointer" data-toggle="modal" data-target="#row_edit_product_price_modal_{{$row_count}}">
			{!! $product_name !!}
		...
		@if(!empty($product->lot_numbers) && empty($is_sales_order))
			<select class="form-inpit input-sm lot_number" name="products[{{$row_count}}][lot_no_line_id]" @if(!empty($product->transaction_sell_lines_id)) disabled @endif>
	...
	<td>
		...
		@if(count($sub_units) > 0)
			<select name="products[{{$row_count}}][sub_unit_id]" class="form-input input-sm sub_unit mt-1">
                @foreach($sub_units as $key => $value)
                    <option value="{{$key}}" data-multiplier="{{$value['multiplier']}}" data-unit_harga="{{$value['harga']}}" data-unit_name="{{$value['name']}}" data-allow_decimal="{{$value['allow_decimal']}}" @if($value['is_main']==1) selected @endif>
	...
	@if(!empty($is_direct_sell))
		...
		<td @if(!$edit_discount) class="hide" @endif>
			{!! Form::text("products[$row_count][line_discount_amount]", @num_format($discount_amount), ['class' => 'form-control  row_discount_amount']); !!}<br>
