@foreach( $variations as $variation)
    <tr @if(!empty($purchase_order_line)) data-purchase_order_id="{{$purchase_order_line->transaction_id}}" @endif @if(!empty($purchase_requisition_line)) data-purchase_requisition_id="{{$purchase_requisition_line->transaction_id}}" @endif>
        ...
        <td>
            @php
                $pp_without_discount = !empty($purchase_order_line) ? $purchase_order_line->unit_price_before_discount/$purchase_order->exchange_rate : $variation->default_purchase_price;

                $discount_percent = !empty($purchase_order_line) ? $purchase_order_line->discount_percent : 0;

                $purchase_price = !empty($purchase_order_line) ? $purchase_order_line->unit_price/$purchase_order->exchange_rate : $variation->default_purchase_price;
        ...
        <td class="{{$hide_tax}}">
            @php
                $dpp_inc_tax = number_format($variation->unit_price_inc_tax, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator);
                if($hide_tax == 'hide'){
                ...
                $dpp_inc_tax = !empty($purchase_order_line) ? number_format($purchase_order_line->unit_price/$purchase_order->exchange_rate, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator) : $dpp_inc_tax;
        ...
        <td class="@if(!session('business.enable_editing_product_from_purchase') || !empty($is_purchase_order)) hide @endif profit_percent_row">
